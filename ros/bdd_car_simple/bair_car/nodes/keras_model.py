...
def init_model():
    ...
    solver = SqueezeNet()
    solver.init(rp.weight_file_path)    
...
def run_model(camera_input, metadata):
    ...
    ai_motor = 100 * output[0][11].data[0] 
    ai_steer = 100 * output[0][2].data[0] 
...
def format_camera_data(left_list, right_list):
    ...
    camera_data = np.concatenate(listoftensors, axis=2)
