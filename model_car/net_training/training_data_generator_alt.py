...
def visualize_data_model(version, data_set, flip):
	if version == 'version_1b':
		return visualize_data_model_1b(data_set, flip)
	if version == 'squeeze_net':
		return visualize_data_model_squeeze_net(data_set,flip)
	assert(False)    
...
def visualize_data_model_1b(data_set,flip):
...
# model squeeze_net
def training_data_generator_squeeze_net(data,flip,show_data=False, camera_dropout=False):
    result = {}
    x_train = {}
    y_train = {}
    #return True
...
def visualize_data_model_squeeze_net(data_set,flip):
    figure_caption = 'ZED_input'
    x_train = data_set['x_train']
    y_train = data_set['y_train']
    r = 2
